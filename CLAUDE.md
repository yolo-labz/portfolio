...
**Tailwind CSS v4**: No `tailwind.config.js`. Theme lives in the `@theme` directive in `apps/web/src/app/globals.css`. Palette is **Catppuccin Mocha, hex-pinned** (NOT oklch, NOT `ctp-*` classes) per the vault's `VISUAL-IDENTITY.md`. Color carries meaning, not decoration: mauve `#cba6f7` = primary/CTA, peach `#fab387` = audit, yellow `#f9e2af` = compliance, blue `#89b4fa` = links. **No gradients, no glows** (banned — the hero uses an engineering-notebook dot-grid instead). The `@source` directive pulls `packages/ui/src` for class detection. Fonts: Inter (sans) + JetBrains Mono (mono) wired via `apps/web/src/lib/fonts.ts` CSS variables. Dark-only (`color-scheme: dark`).
