...
- **`projects/*`** — 2 self-contained portfolio projects, each a thin `package.json` wrapper (Turbo orchestration only) over its own language tooling. Biome ignores `*.py`; Python uses ruff per-project. Workspace globs also pull in `projects/*/site` and `projects/*/dashboard` as real Next.js workspaces.
